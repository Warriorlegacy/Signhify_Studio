...
export const Route = createFileRoute("/marketplace/success")({
  validateSearch: (s) => ({
  ...
  head: () => ({
    meta: [
      { title: "Marketplace Success — Signhify | AI Product Studio" },
      {
        name: "description",
        content:
...
function SuccessPage() {
  const { session_id, listing_id } = Route.useSearch();
  const verify = useServerFn(verifyCheckoutSession);
  const dl = useServerFn(downloadAsset);
  ...
  return (
    <section className="pt-36 pb-24 min-h-screen px-6">
      <div className="mx-auto max-w-2xl rounded-3xl border border-border bg-card p-8 text-center">
        <h1 className="font-display text-4xl font-black">Marketplace checkout</h1>
        {err && <p className="mt-4 text-red-300">{err}</p>}
        {paid ? (
          <>
            <div className="mt-6 rounded-xl border border-emerald-500/40 bg-emerald-500/10 p-4 text-emerald-300">
              Payment confirmed. Your download is unlocked.
            </div>
            <button
              onClick={async () => {
                const { signedUrl } = await dl({ data: { listingId: listing_id } });
