...
function keyFrom(masterKey: string) { return crypto.createHash("sha256").update(masterKey).digest(); }
...
export function decryptAES256GCM(ciphertext: string, masterKey: string): string {
  ...
  const decipher = crypto.createDecipheriv("aes-256-gcm", keyFrom(masterKey), Buffer.from(ivHex, "hex"));
