...
async function stripe(path: string, body: URLSearchParams) {
  ...
  try {
    ...
    logger.error(`Stripe call error: ${error}`);
    throw error;
  }
}
...
export const createSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      ...
      logger.info(`Created checkout session for priceId: ${data.priceId}`);
      return { url: session.url as string };
    } catch (error) {
      logger.error(`Failed to create subscription checkout: ${error}`);
      throw error;
    }
  });
