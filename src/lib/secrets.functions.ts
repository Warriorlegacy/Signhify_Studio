...
export const listSecretKeys = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => ({ projectId: String((i as any)?.projectId ?? "") }))
...
export const createSecret = createServerFn({ method: "POST" })
  ...
  .inputValidator((i: unknown) => ({
    projectId: String((i as any)?.projectId ?? ""),
    key: String((i as any)?.key ?? "").trim(),
    value: String((i as any)?.value ?? ""),
  }))
  .handler(async ({ data, context }) => {
    ...
    const { error } = await supabase.from("project_secrets").insert({
      ...
      user_id: userId,
      key: data.key,
      encrypted_value: encryptAES256GCM(data.value, masterKey),
    });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
export const deleteSecret = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) => ({
    secretId: String((i as any)?.secretId ?? ""),
    projectId: String((i as any)?.projectId ?? ""),
  }))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context as any;
