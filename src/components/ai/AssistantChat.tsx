...
const MODELS: AIModel[] = [
  { id: "gpt-5.6-luna", name: "GPT-5.6 Luna", provider: "OpenAI", badge: "Latest" },
  { id: "claude-3.7-sonnet", name: "Claude 3.7 Sonnet", provider: "Anthropic", badge: "Smart" },
  { id: "gemini-3-flash", name: "Gemini 3.0 Flash", provider: "Google", badge: "Fast" },
  { id: "deepseek-v3.1", name: "DeepSeek V3.1", provider: "DeepSeek", badge: "Open" },
];
...
export function AssistantChat({ className = "" }: { className?: string }) {
  ...
  const handleSend = (textToSend?: string) => {
    ...
    // Simulate real stream response or hook into backend
    setTimeout(() => {
      const assistantMsg: ChatMessage = {
        id: "ast_" + Date.now(),
        role: "assistant",
        content: `Here is the architectural execution for: "${query}"\n\n\`\`\`typescript\n// Generated by ${selectedModel.name}\nexport function useOptimizedFeature() {\n  return {\n    status: "active",\n    stack: ["React 19", "Tailwind CSS", "Framer Motion", "TanStack Query"],\n    timestamp: new Date().toISOString()\n  };\n}\n\`\`\`\n\nThis implementation follows the strict Signhify high-end visual design guidelines with double-bezel enclosures and fluid spring micro-interactions.`,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {isStreaming && (
          <div className="flex gap-4 items-center text-xs text-zinc-400">
            ...
            <span className="animate-pulse">{selectedModel.name} is streaming response...</span>
          </div>
        )}
          ...
          <div className="flex items-center justify-between px-2 pt-1 border-t border-white/5">
            ...
            <div className="relative">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium bg-zinc-800/80 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-white/5 transition-colors"
              >
              ...
              <AnimatePresence>
                {isModelDropdownOpen && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 5 }}
                    className="absolute bottom-full mb-2 left-0 w-52 rounded-xl bg-zinc-900 border border-white/10 shadow-2xl p-1.5 z-50 backdrop-blur-xl"
                  >
                    {MODELS.map((model) => (
                      <button
                        key={model.id}
                        ...
                        className={`w-full flex items-center justify-between px-2.5 py-2 rounded-lg text-xs transition-colors ${
                          selectedModel.id === model.id ? "bg-white/10 text-white font-semibold" : "text-zinc-400 hover:bg-white/5 hover:text-white"
                        }`}
                      >
