...
const STAGES: Stage[] = ["briefing", "architecture", "design_tokens", "codegen", "review", "deploy_plan"];
const corsHeaders = {
...
function sse(payload: unknown) { return `data: ${JSON.stringify(payload)}\n\n`; }
function stageFromText(text: string, current: Stage): Stage {
  const lower = text.toLowerCase();
  for (const stage of STAGES) if (lower.includes(`[${stage}]`) || lower.includes(stage.replace("_", " "))) return stage;
  return current;
}
...
serve(async (req) => {
  ...
  if (typeof prompt !== "string" || prompt.trim().length < 3) return new Response(JSON.stringify({ error: "Prompt is required" }), { status: 400, headers: { ...corsHeaders, "content-type": "application/json" } });

  const ip = ipFrom(req);
  const windowStart = currentWindow();
  const { data: existing } = await supabase.from("rate_limits").select("count").eq("ip", ip).eq("window_start", windowStart).maybeSingle();
  if ((existing?.count ?? 0) >= 10) return new Response(JSON.stringify({ error: "Rate limit exceeded. Try again later." }), { status: 429, headers: { ...corsHeaders, "content-type": "application/json" } });
  await supabase.from("rate_limits").upsert({ ip, window_start: windowStart, count: (existing?.count ?? 0) + 1 });
  ...
  if (!anth.ok || !anth.body) return new Response(JSON.stringify({ error: "Anthropic request failed" }), { status: anth.status || 502, headers: { ...corsHeaders, "content-type": "application/json" } });
